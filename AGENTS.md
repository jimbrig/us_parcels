...
- Hilbert-sorted, ZSTD compressed, GeoArrow geometry encoding, 50k row groups.
- GeoArrow encoding (not WKB) means geometry is stored as a native Arrow STRUCT type. DuckDB spatial functions (ST_Hilbert etc.) CANNOT operate on GeoArrow STRUCT columns directly — use `centroidx`/`centroidy` columns for Hilbert ordering instead.
- freestiler's R DuckDB fallback CANNOT read GeoArrow geometry — export TIGER and context data with WKB encoding (`-lco GEOMETRY_ENCODING=WKB` or omit `GEOMETRY_ENCODING=GEOARROW`) when the data will be fed to freestiler.
- Do NOT re-extract from the GPKG if a state parquet already exists. Use it as source for all derivative operations (county partitioning, PMTiles, analytics).
- County-level partitions live at `data/geoparquet/state=XX/county=YYY/parcels.parquet` (generated by `pixi run partition-county -- --state XX`).

Completed states: GA (13) — 6.96M features, 159 counties. NC (37) — 5.76M features, 100 counties.
